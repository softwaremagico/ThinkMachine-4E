 ...
 * equipment, and abilities. Adapted from Fading Suns 3E to work with 4E character model.
 */
public class ThreatLevel {
	 ...
	 *
	 * @param characterPlayer the character to evaluate
	...
	private static int getThreatLevel(Weapon weapon) {
		try {
			...
			// Threat from damage
			try {
				final int mainDamage = weapon.getWeaponDamages().get(0).getMainDamage();
	...
	private static int getCyberneticsThreatLevel(CharacterPlayer characterPlayer) {
		try {
			...
			for (final Cyberdevice cyberdevice : cyberdevices) {
				...
				final String name = cyberdevice.getName() != null ? cyberdevice.getName().getTranslatedText().toLowerCase() : "";
	...
	/**
	 * Detects if a cyberdevice is combat-related by examining its id, name, and description.
	 *
	...
	private static boolean isCombatRelated(String id, String name, String description) {
		...
		final String[] combatKeywords = {
			"combat", "fight", "attack", "weapon", "armor", "shield",
			"weapon system", "targeting", "sniper", "cannon", "gun",
			"blade", "sword", "enhanced strength", "enhanced durability",
	...
	public static void showStatistics() {
		if (total > 0) {
			...
			System.out.println("Resistance threat: " + resistanceThreatLevel / total);
		}
	}
