...
public class FactionFactoryTests {
	...
	public void checkTotalElementsOnlyOnFactionBook() throws InvalidXmlElementException {
		...
		Assert.assertEquals(FactionFactory.getInstance().getElements().size(), DEFINED_FACTIONS_FACTION_BOOK);
	}
	...
	public void checkTotalElementsOnlyOnFS4E() throws InvalidXmlElementException {
		...
		FactionFactory.getInstance().reset();
		Assert.assertEquals(FactionFactory.getInstance().getElements().size(), DEFINED_FACTIONS_FS_4E);
	}
	...
	public void readCustomizedWeapon() throws InvalidXmlElementException {
		...
		Assert.assertEquals(
				hawkwood.getMaterialAwards().get(0).getOptions().iterator().next().getElement().getQuality(),
				Quality.PREMIUM);
	}
	...
	public void readBlessedWeapon() throws InvalidXmlElementException {
		...
		Assert.assertTrue(brotherBattle.getMaterialAwards().get(0).getOptions().iterator().next()
				.getElement() instanceof CustomizedWeapon);
		Assert.assertEquals(
				brotherBattle.getMaterialAwards().get(0).getOptions().iterator().next().getElement().getStatus(),
				Status.BLESSED);
	}
	...
	public void itemTechConvulsion() throws InvalidXmlElementException {
		...
		Assert.assertEquals(
				engineers.getMaterialAwards().get(0).getOptions().iterator().next().getElement().getTechCompulsion(),
				"industrious");
	...
	@Test(dependsOnMethods = "getWeaponQuality")
	public void validateFactionBookFactionsWithItemsAndCallings() throws InvalidXmlElementException {
		this.enableAllModules();

		this.assertFactionMaterialAwardsAndFavoredCallings("keddah", Set.of("pocketPavilion"), "explorer");
		this.assertFactionMaterialAwardsAndFavoredCallings("courtesans", Set.of("elegantCourtesanMask"), "courtier");
		this.assertFactionMaterialAwardsAndFavoredCallings("slayers", Set.of("shimmercloak", "stonefoot"), "spy");
		this.assertFactionMaterialAwardsAndFavoredCallings("townCriers", Set.of("mojoRecorder"), "amateur");
		this.assertFactionMaterialAwardsAndFavoredCallings("wordwrights", Set.of("mojoPaper"), "artist");
		this.assertFactionMaterialAwardsAndFavoredCallings("preceptors", Set.of("portableLibrary"), "scholar");
		this.assertFactionMaterialAwardsAndFavoredCallings("chorali", Set.of("hymnalBook"), "chorister");
		this.assertFactionMaterialAwardsAndFavoredCallings("hesychasts", Set.of("blessedRelic"), "mendicant");
		this.assertFactionMaterialAwardsAndFavoredCallings("incarnates", Set.of("secretCodeNotes"), "friar");
		this.assertFactionMaterialAwardsAndFavoredCallings("childrenOfZuran", Set.of("libroEsperanza"), "trader");
		this.assertFactionMaterialAwardsAndFavoredCallings("gjarti", Set.of("knottedWoodPendant"), "theurgist");
	}
	...
	private void assertFactionMaterialAwardsAndFavoredCallings(String factionId, Set<String> expectedItemIds,
			String... expectedCallings) throws InvalidXmlElementException {
		final Faction faction = FactionFactory.getInstance().getElement(factionId);

		Assert.assertNotNull(faction);
		Assert.assertEquals(
				faction.getMaterialAwards().stream().flatMap(materialAward -> materialAward.getOptions().stream())
						.map(equipmentOption -> equipmentOption.getElement().getId()).collect(Collectors.toSet()),
				expectedItemIds);
		Assert.assertEquals(faction.getFavoredCallings().stream().collect(Collectors.toSet()),
				Set.of(expectedCallings));
		for (final String favoredCalling : expectedCallings) {
			Assert.assertNotNull(CallingFactory.getInstance().getElement(favoredCalling));
		}
	}
}
