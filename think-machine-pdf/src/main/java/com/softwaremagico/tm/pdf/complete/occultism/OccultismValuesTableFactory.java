...
public class OccultismValuesTableFactory extends BaseElement {
    ...
    private static PdfPTable createPsiTable(CharacterPlayer characterPlayer) {
        ...
        table.addCell(createContent(characterPlayer, CharacteristicName.PSI));
        table.addCell(createContent(characterPlayer, CharacteristicName.URGE));
    ...
    private static PdfPTable createTheurgyTable(CharacterPlayer characterPlayer) {
        ...
        table.addCell(createContent(characterPlayer, CharacteristicName.THEURGY));
        table.addCell(createContent(characterPlayer, CharacteristicName.HUBRIS));

        return table;
    }

    public static PdfPCell createContent(CharacterPlayer characterPlayer, CharacteristicName characteristicName) {
        final float[] widths = {3f, 1.5f, 0.1f};
        final PdfPTable table = new PdfPTable(widths);
